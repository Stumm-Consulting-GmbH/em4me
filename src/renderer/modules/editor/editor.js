...
// 4T-001002 (Epic 3E-000196): Linter, Tastenbelegungen, Einfuege-Handler und
...
// 4T-000294: Linter und Typewriter-Scroll sind schaltbare Erweiterungen.
import { isExtensionActive } from '../extensions/extension-lifecycle.js';
// 4T-000581 (Epic 3E-000107): Schalter-Aufloesung der Rechtschreibpruefung.
import { SPELLCHECK_EXTENSION_ID, spellcheckAttributeValue } from '../../../shared/spellcheck.js';
// 4T-000589 (Epic 3E-000109): Die reinen Pipe-Tabellen-Text-Helfer leben im
...
// 4T-000655 (Epic 3E-000112): Nummerierungs-Invariante beider Editor-Zustaende
// (Nutzung nur in Funktionskörpern, Laufzeit-Zyklus unkritisch).
import { listRenumberFilter } from './editor-list-tools.js';
// 4T-001002 (Epic 3E-000196): Tastenbelegungen des Editors. Laufzeit-Zyklus ueber
...
// 4T-001002: Markdown-Linter. Laufzeit-Zyklus, Zugriffe nur in Funktionskoerpern.
import { lintField, lintHoverTooltip, lintUpdateListener, scheduleLint } from './editor-lint.js';
// 4T-001002: Einfuege- und Anlagen-Handler. Laufzeit-Zyklus, Zugriffe nur in
// Funktionskoerpern.
import { imageOpenHandler, pasteLinkHandler } from './editor-paste.js';
// 4T-001002: Vorschau-Aufbau des Render-Pane.
...
// 4T-000364 (Epic 3E-000067): Cursor-Folge und Doc-Aenderung des Block-
...
// 4T-000486 (Epic 3E-000091): Schreib-Trigger "\\" oeffnet den Datums-/
// Uhrzeit-Picker (Erweiterungs-Gate liegt im Handler selbst).
// 4T-000487 (PO-Befund Runde 1): dateValuePlugin dekoriert klickbare
...
// 4T-000607 (Epic 3E-000114): Format-Toolbar — Sichtbarkeit folgt dem
...
// 4T-000585 (Epic 3E-000108): Titelzeile — Sichtbarkeit und Text folgen dem
...
export const editorCompartments = {
  ...
  // 4T-000088 (Epic 3E-000014): basePath fuer Block-Widgets (Tabellen, Code,
  ...
  // 4T-000013: Gliederung (Folding-Gutter inkl. Struktur-Field und Width-Sync)
  ...
  // 4T-000019: Typewriter-Scroll als Compartment, damit der Listener zur
  // Laufzeit ein-/ausgeschaltet werden kann, ohne den Editor neu aufzubauen.
  typewriter: new Compartment(),
  // 4T-000080 (Epic 3E-000014): Live-Preview-Plugin per Compartment, pro
  ...
  // wurde mit 4T-000085 durch den vierten View-Modus abgeloest.
  livePreview: new Compartment(),
  // 4T-000207 (Epic 3E-000015): Registry-gespeiste Keymap der editorScoped-
  // Kommandos (Fold). Compartment, damit 4T-000208 bei hotkeys:changed zur
  // Laufzeit rekonfigurieren kann, ohne den Editor neu aufzubauen.
  commandKeymap: new Compartment(),
  // 4T-000581 (Epic 3E-000107): spellcheck-Attribut der Editor-Flaeche. Das ist
...
// 4T-000398 (Epic 3E-000066): Schlanke EditorState-Factory fuer das Notizen-Feld.
...
export function createNotesEditorState({ content = '', placeholderText = '', onDocChanged } = {}) {
  return EditorState.create({
    ...
    extensions: [
      ...
      // 4T-000603 (Epic 3E-000113): eingebauten lang-markdown-Paste-Handler
      ...
      // 4T-000603 (Epic 3E-000113): URL-in-Auswahl als Markdown-Link auch im
      ...
      EditorView.updateListener.of((update) => {
        if (update.docChanged) {
          // 4T-000526 (Epic 3E-000095): Tipp-Aktivitaet fuer die Tipp-Ruhe
...
// 4T-000019: Typewriter-Scroll-Extension. Bei jeder Cursor- oder Selektions-
...
// 4T-000179: foldGutterExtensions nach folding.js verschoben (Wert-
// Einbettung der Gutter-Plugins; Body-Reihenfolge der Modul-Zyklen).

// 4T-000935 (Befund B-08): Der geschriebene Stand einer offenen Datei wandert
...
// gemeldet. Der Fehler lag im Bestand von 4T-000935 und traf alle Verbraucher
...
//
// 4T-000948: Abgeraeumt wird nur der Plan DIESER Datei. Vorher traf es den
...
export function createEditorState(opts = {}) {
  return EditorState.create({
    ...
    extensions: [
      ...
      // 4T-000013: Heading-Folding mit Hierarchie-Spuren. Der eigene
      ...
      // 4T-000207 aus der Kommando-Registry (commandKeymap-Compartment unten).
      codeFolding(),
      // foldStructureField dauerhaft aktiv, weil das Outline-Panel (4T-000014)
      ...
      // 4T-000019: Typewriter-Scroll als Compartment, zur Laufzeit togglebar.
      // 4T-000294: nur bei aktiver Fokus-Modus-Erweiterung (die persistierte
      ...
      haengenderEinzugPlugin,
      history(),
      // 4T-000640 (Epic 3E-000069): Schreibschutz-Wache. Sie steht bewusst vor
      ...
      EditorState.transactionFilter.of(listRenumberFilter),
      // 4T-000074: Tab/Shift-Tab/Enter in Pipe-Tabellen — VOR dem Listen-Indent
      ...
      tableEditKeymap,
      // 4T-000016: Tab/Shift-Tab fuer Listen-Indent vor dem defaultKeymap.
      listIndentKeymap,
      // 4T-000656: Tabulator ausserhalb von Listen und Tabellen (einstellbar).
      // Ohne erhoehte Praezedenz, damit die beiden Handler darueber Vorrang
      // behalten.
      tabIndentKeymap,
      // 4T-000207: Fold-Bindings aus der Registry (vorher statisches
      ...
      lintUpdateListener,
      lintHoverTooltip,
      // 4T-000577 (Epic 3E-000106): Hervorhebung der Cursor-Zeile (Editor und
      ...
      drawSelection(),
      searchHighlightField,
      // 4T-000049: YAML-Frontmatter im Source-Pane visuell abgesetzt darstellen.
      frontmatterField,
      // 4T-000061: Callout-Header-Marker `[!type][+-]?` visuell abheben.
      calloutMarkerField,
      // 4T-000062: Highlight-Inhalt zwischen `==…==` gelb hinterlegen.
      markMarkerField,
      // 4T-000063: Footnote-Referenzen und -Definitionen markieren.
      footnoteMarkerField,
      // 4T-000479: %%-Kommentar-Bereiche dezent einfaerben.
      commentMarkerField,
      // 4T-000596: Inline-Berechnungen {= … =} dezent einfaerben (beide Modi).
      inlineCalcMarkerField,
      // 4T-000057: Autocomplete fuer Wiki-Links und Tags.
      autocompleteExtension,
      // 4T-000486: Schreib-Trigger "\\" fuer den Datums-/Uhrzeit-Picker.
      datePickerTriggerExtension,
      // 4T-000487: klickbare Datums-/Uhrzeit-Werte (Quelltext- und Live-Modus).
      dateValuePlugin,
      // 4T-000546: klickbare Kalender-Werte @{Name: Wert} (beide Modi).
      calendarValuePlugin,
      // 4T-000943: Sichtbarkeit des Strg-Zugangs in der aktiven Zeile.
      modifierZustandExtension,
      // 4T-000603 (Epic 3E-000113): URL-in-Auswahl als Markdown-Link einfügen.
      pasteLinkHandler,
      // 4T-000790 (Epic 3E-000125): Doppelklick auf ein Bild oeffnet die Anlage.
      imageOpenHandler,
      EditorView.updateListener.of((update) => {
        ...
        if (update.docChanged) {
          ...
          editorActivity.lastDocEditAt = Date.now();
          // R2-11 (4T-000180): geteilte Serialisierung (eine Voll-Text-Kopie
          ...
          scheduleAutoSave();
          // 4T-000935 (Befund B-08): geschriebenen Stand an den Index-Overlay
          ...
          // R5-01 (4T-000171): Doc-Aenderung macht die Source-Such-Offsets
          ...
          // 4T-000014: Outline rendert bei jeder Doc-Aenderung neu (Debounce
          // 200 ms), damit die Hierarchie immer aktuell ist.
          if (state.outline.visibleByPane[pIdx]) scheduleOutlineRender(pIdx);
          // 4T-001047: Mindmap folgt mit demselben Debounce (zeichnet nur im Modus).
          ...
          // 4T-000364 (Epic 3E-000067): Anker-Liste und Verwaisten-Abgleich des
        ...
        // 4T-000014: Cursor-Bewegung triggert Aktiv-Sektion-Update mit
        ...
        // 4T-000364 (Epic 3E-000067): Cursor-Bewegung laesst das Block-
        ...
        // 4T-000072: Selektionswechsel aktualisiert die Word-Count-Anzeige
        ...
        // 4T-000607 (Epic 3E-000114): Cursor-/Dokument-Aenderungen ziehen die
...
export function ensureEditorForPane(paneIdx) {
  ...
  // 4T-000377 (Epic 3E-000071): Rechtsklick öffnet das Editor-Kontextmenü.
  ...
  // 4T-000070: Scroll-Sync-Listener auf beide Panes; aktiv nur, wenn der
...
export function syncEditorForPane(paneIdx) {
  ...
  if (!pane || pane.activeIndex < 0) {
    ...
    // 4T-000341: ohne aktiven Tab keine Breadcrumb-Leiste.
    updateSubpageBreadcrumb(paneIdx);
    // 4T-000607 (Epic 3E-000114): ohne aktiven Tab keine Format-Toolbar.
    updateFormatToolbarForPane(paneIdx);
    // 4T-000585 (Epic 3E-000108): ohne aktiven Tab keine Titelzeile.
    updateTitleLineForPane(paneIdx);
    return;
  }
  const tab = pane.tabs[pane.activeIndex];
  if (!tab) return;
  // 4T-001325 (Epic 3E-000236): Inhalt und Pfad in EINER Transaktion — getrennt gefahren
  ...
  view.dispatch({
    ...
    effects: [
      ...
      // 4T-000088: basePath an den neuen Tab anpassen. Block-Widgets im
      ...
      // 4T-000085: Live-Preview pro Tab. Compartment traegt das
      // livePreviewExtensions-Bundle nur, wenn tab.viewMode === 'live'.
      // 4T-000365 (Epic 3E-000067): der Block-Metadaten-Indikator (blockMetaField)
  ...
  updateFormatToolbarForPane(paneIdx);
  // 4T-000585 (Epic 3E-000108): Titelzeile folgt Tab und Ansichts-Modus.
  updateTitleLineForPane(paneIdx);
  // R2-14 (4T-000180): Nachhol-Lint fuer Editor-Modi. runLint bricht im
  ...
  // 4T-000365 (Epic 3E-000067): im Live-Modus die Block-Metadaten der Datei laden,
  ...
  // 4T-000014: Bei Tab-Wechsel die Outline der Pane an die neue Heading-
  ...
  // 4T-000015: Bei Tab-Wechsel die Backlinks neu anfordern, falls Sektion
  ...
  // 4T-000051: Bei Tab-Wechsel Properties-Sektion neu rendern, falls sichtbar.
  if (state.properties && state.properties.visibleByPane[paneIdx]) {
    renderProperties(paneIdx);
  }
  // 4T-000056: Tag-Sektion ebenfalls aktualisieren. Filter (filterByPane) und
  ...
  // 4T-000341 (Epic 3E-000061): Breadcrumb und Unterseiten-Sektion folgen dem
...
// 4T-000013: Read-/Write-API fuer Heading-Folding zur Verwendung durch das
// Outline-Panel (4T-000014). Liest den Folding-Status einer Heading-Zeile bzw.
...
// aktiven Tab. 4T-000318 (Epic 3E-000057): der Klammer-Suffix ist gestuft —
...
export function updateWindowTitle() {
  ...
  const info = {
    ...
    // 4T-000871 (Buch = Bereich): Titel-Stufe "Buch {name}"; bei
    ...
    // 4T-000873: Regal-Stufe, dieselbe Erweiterung wie die Bücher.
  ...
  // 4T-001044 (Weg A): Suffix erst ab geladenem Woerterbuch, sonst stuende der rohe

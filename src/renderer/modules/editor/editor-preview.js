...
//
// Auszug aus editor.js, 4T-001002 (Epic 3E-000196).
'use strict';
...
// R2-01 (4T-000174): Preview-Debounce pro Pane (vorher global — ein Tipp in
...
// 4T-000653: Laeuft im Render-Pane gerade eine Inline-Bearbeitung, darf der
...
// 4T-000653: Bricht einen geplanten Vorschau-Aufbau ab. Fuer Komponenten, die
...
export function schedulePreviewUpdate(paneIdx) {
  ...
  // R2-12 (4T-000180): adaptiver Debounce — der Voll-Render kostet bei
  ...
  pendingPreviewTimers[paneIdx] = setTimeout(() => {
    pendingPreviewTimers[paneIdx] = null;
    // 4T-000653: Aufbau aufschieben statt die offene Bearbeitung zu zerstoeren.
...
export function renderPreviewForPane(paneIdx) {
  ...
  // R2-12 (4T-000180): fertig geladene PDF-Embed-Knoten zwischen zwei
  ...
  // (enthaelt seit 4T-000324 auch die Aussen-Link-Warnung der Bereichs-Apps).
  applyRenderPipeline(els.renderedHtml, tab.path);
  // R4-12 (4T-000180): Render-Skip-Cache nachfuehren — das DOM entspricht
  // jetzt diesem Tab-Stand, der naechste renderPaneContent kann skippen.
  notePaneRendered(paneIdx, tab);
  // 4T-000014: Aktive Outline-Sektion neu ermitteln, weil DOM-Heading-Knoten

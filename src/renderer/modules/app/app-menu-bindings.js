...
export function bindMenuEvents() {
  ...
  // 4T-000002 nicht mehr in der UI).
  ...
  // 4T-000887 (Befund L-04): 'Buch und Buecherregal -> Kapitel-Datei
  ...
  // 4T-000538 (Epic 3E-000098): Arbeitsbereichs-Aktionen aus dem Datei-Menue
  ...
  // 4T-000338 (Epic 3E-000061): 'Datei -> Neue Unterseite...'.
  api.onMenuNewSubpage(() => createSubpageForActiveFile());
  // 4T-000426 (Epic 3E-000080): 'Datei -> Neue Datei aus Vorlage...'.
  api.onMenuNewFromTemplate(() => newFileFromTemplate());
  // 4T-000433 (Epic 3E-000081): 'Datei -> Heutiger Journal-Eintrag' und
  ...
  // 4T-000339 (Epic 3E-000061): 'Datei -> Umbenennen...' plus zentraler
  // Nachzug nach einem Umbenennen (Broadcast aus dem Main).
  api.onMenuRenameFile(() => renameActiveFile());
  // 4T-000774 (Epic 3E-000128): 'Datei -> Von der uebergeordneten Seite loesen...'.
  api.onMenuDetachSubpage(() => detachActiveSubpage());
  // 4T-001293 (Epic 3E-000224): 'Datei -> Teile wieder vereinen...'.
  ...
  // 4T-000345 (Epic 3E-000062): angewendetes Link-Update nachziehen (nicht-dirty
  ...
  // 4T-000216: Hilfe-Menue-Eintrag oeffnet die Handbuch-Ueberblicksseite.
  api.onMenuOpenHelp(() => openManualPage('overview'));
  api.onMenuOpenAbout(() => showAbout());
  // 4T-000644 (Epic 3E-000127): Hilfe -> Produkt-Tour startet die geführte Tour
  // (parameterloser Aufruf = manueller Start).
  if (typeof api.onMenuStartTour === 'function') {
    api.onMenuStartTour(() => startTour());
  }
  // 4T-000277: Menue-Eintrag Datei -> Einstellungen oeffnet die Seite.
  if (typeof api.onMenuOpenSettings === 'function') {
    api.onMenuOpenSettings(() => openSettingsPage());
  }
  // 4T-000333 (Epic 3E-000060): Ansicht -> Dokument-Historie oeffnet die
  ...
  // 4T-000620 (Epic 3E-000117): Ansicht -> Bereichs-Statistik oeffnet die
  ...
  // 4T-000697 (Epic 3E-000141): Menue-Eintraege "Linke/Rechte Sidebar einklappen"
  ...
  // 4T-000019: Bearbeiten-Toggle ueber das Ansicht-Menue (Strg+E). Loest den
  ...
  // 4T-000018: Multi-Window-Broadcast: ein anderes Fenster hat eine appearance.*-
  ...
  if (typeof api.onAppearanceChanged === 'function') {
    api.onAppearanceChanged((values) => {
      ...
      // R5-08 (4T-000177): offenen Settings-Snapshot mitziehen, sonst
  ...
  // 4T-000207: gemeinsamer Toggle-Pfad mit dem Kommando app.toggleRestoreSession.
  api.onMenuToggleRestoreSession(() => toggleRestoreSessionSetting());
  // 4T-000014: Menue-Eintrag "Ansicht -> Inhaltsverzeichnis" toggelt die
  ...
  // 4T-000015: Menue-Eintrag "Ansicht -> Backlinks" und Live-Update-Listener.
  ...
  // 4T-000073: Menue-Eintrag "Ansicht -> Outgoing-Links".
  ...
  // 4T-000075: "Datei -> Lesezeichen -> Aktive Datei merken" (Strg+D) und
  ...
  if (typeof api.onBacklinksInvalidated === 'function') {
    api.onBacklinksInvalidated(() => {
      ...
      for (let i = 0; i < state.panes.length; i++) {
        ...
        // 4T-000341 (Epic 3E-000061): sichtbare Unterseiten-Sektionen folgen
        // Datei-Anlagen/-Loeschungen im Suchraum.
        if (state.subpages && state.subpages.visibleByPane[i]) scheduleSubpagesRender(i);
        // B-20 (4T-000187): sichtbare Tag-Sektionen ebenfalls aktualisieren —

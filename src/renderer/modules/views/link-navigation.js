...
// 4T-000336 (Epic 3E-000061): Unterseiten-Namens-Logik — Expansion relativer
...
// 4T-000204: Toggle-Pfad der Status-Zeichen; seit 4T-000497 folgt der Klick
...
export async function activateLink(paneIdx, href, isWikilink, baseOverride) {
  ...
  if (href.startsWith('#')) {
    // 4T-000054: Anker im selben Dokument. K-02 (4T-000186): modusbewusst —
  ...
  // 4T-000054: Pfad und Anker trennen. Nach dem Oeffnen der Ziel-Datei
  ...
  // R2-02 (4T-000174): Links in Markdown-Embeds loesen gegen die Embed-Datei
  // auf (baseOverride aus data-embed-base), nicht gegen den Pane-Tab.
  const basePath = baseOverride || baseTab.path;
  // 4T-000336 (Epic 3E-000061): relative Unterseiten-Links ('/Name.md', '..')
  ...
  if (!exists) {
    if (isWikilink && basePath) {
      // 4T-000337 (Epic 3E-000061): deterministischer Versuch ohne Index —
      ...
      if (/[/\\]/.test(effectivePath)) {
        ...
        if (cand && (await api.fileExists(cand))) {
          // 4T-000631 (Epic 3E-000102): Link-Klicks im Dokument-Inhalt erben die
      ...
      // B-13 (4T-000175): Suchraum-Fallback ueber den Backlinks-Index —
      ...
      // 4T-000336: volle logische Ziel-Form statt nur des Basenames — der
      ...
      try {
        ...
        if (idx && idx.status === 'ready' && idx.candidates.length > 0) {
          ...
          if (target) {
            // R4-09 (4T-000186): tatsaechliche Ziel-Pane verwenden — die
      ...
      // 4T-000050 (Epic 3E-000010): Alias-Fallback. Eindeutiger Treffer oeffnet
  ...
  if (!isMd) {
    // 4T-000790 (Epic 3E-000125): Ein Verweis auf eine Nicht-Markdown-Datei blieb
  ...
  // R4-09 (4T-000186): tatsaechliche Ziel-Pane verwenden.
...
export async function handleRenderedClick(e, paneIdx) {
  // K-11 (4T-000186): Task-Checkbox-Klick im Render-Pane — toggelt den
  // Marker im Quelltext (Paritaet zum Live-Modus-Task-Toggle).
  if (e.target instanceof HTMLInputElement && e.target.type === 'checkbox') {
    toggleTaskFromRendered(paneIdx, e.target);
    return;
  }
  // 4T-000204: Klick auf eine erweiterte Status-Box — gleiche Mechanik,
  ...
  // 4T-000355 (Epic 3E-000065): Klick auf einen Frontmatter-Abfrage-Eintrag öffnet
  ...
  // Einträge selbst <a href="#"> sind. 4T-000409 (Epic 3E-000077): Block-Treffer
  ...
  // 4T-000504 (Epic 3E-000096): Rueckschreib-Aktionen der Task-Treffer (Status-
  ...
  if (fmItem && fmItem.dataset.fmPath) {
    ...
    // 4T-000502 (Epic 3E-000096): Task-Treffer tragen die Quell-Zeile — nach dem
  ...
  // 4T-000365 (Epic 3E-000067): Klick auf den Block-Metadaten-Indikator öffnet das
  ...
  if (!href) return;
  e.preventDefault();
  // R2-02 (4T-000174): Klicks innerhalb eines Markdown-Embeds tragen die
...
// K-11 (4T-000186): Klick auf eine Task-Checkbox im Render-Pane. Das <li>
...
export function toggleTaskFromRendered(paneIdx, checkboxEl) {
  ...
  // 4T-000213: im read-only Handbuch-Tab bleibt der Status-Klick inert
  ...
  // 4T-000497: der Klick folgt der konfigurierten Toggling-Kette (Basis
  // `[ ]` <-> `[x]` fest, erweiterte Status auf ihr Folge-Symbol); der
  // Dispatch samt Undo-Haertung (4T-000484) liegt in performStatusToggle.
  const toggle = performStatusToggle(view, ln);
  if (!toggle) return;
  // 4T-000204: sobald eine Status-Box beteiligt ist (Quelle oder Ziel),
...
// 4T-000050: Hilfsfunktion fuer den Wiki-Link-Alias-Fallback. Bekommt den

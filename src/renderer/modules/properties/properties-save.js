...
// 4T-000981 (Epic 3E-000196): Auszug aus properties-tags.js. Liegt bewusst
...
// 4T-001179 (Epic 3E-000220): `isEmptyPropertyValue` wird hier nicht mehr
...
// 4T-001172 (Epic 3E-000220): Die Regel, wann ein nur definiertes Feld draußen
...
// 4T-000051: Liest die Felder aus der Sidebar-Sektion einer Spalte und baut
...
export function readPropertiesFromPane(paneIdx) {
  ...
  for (const fieldEl of fieldEls) {
    ...
    if (!key) continue;
    // 4T-001185 (Epic 3E-000221, E1): Ein abgeleitetes Feld geht NIE ins Dokument.
    ...
    // 4T-000448: der Editor-Typ steht in dataset.currentType — bei einem
    ...
    // 4T-001172 (Epic 3E-000220): Ein Feld aus dem Ausklapp-Bereich des
...
// 4T-000051: Plant einen Speichervorgang fuer die Properties-Sektion einer
...
export function scheduleSavePropertiesFromPane(paneIdx) {
  ...
  // R5-03 (4T-000172): Ziel-Tab beim Schedulen merken. Der Save liest die
...
// R5-03 (4T-000172): Pending Debounce-Save sofort ausfuehren, bevor die
...
export function savePropertiesFromPane(paneIdx, targetTab) {
  ...
  // R5-02 (4T-000172): Bei YAML-Parse-Fehler niemals schreiben —
  ...
  // R5-10 (4T-000172): Doppelte Keys nicht still zusammenlegen. Betroffene
  ...
  // 4T-000051: Properties-Aenderung verhaelt sich wie eine Editor-Aenderung.
  ...
  // R5-03 (4T-000172): UI-Refresh nur, wenn der geschriebene Tab noch der
  ...
  // 4T-000448: weiche Hinweise der definierten Felder live nachziehen (ohne
...
// 4T-000448: Hinweis-Icons aller definierten Felder gegen den aktuellen
...
// R5-10 (4T-000172): Markiert Felder mit doppelten Keys (exakter Vergleich
...
export function markDuplicatePropertyKeys(paneIdx) {
  ...
  // 4T-001179 (Epic 3E-000220): Gezählt wird, was ins Dokument geht. Ein leeres
  ...
  for (const fieldEl of els.propertiesFields.querySelectorAll('.properties-field')) {
    // 4T-001185: Ein abgeleitetes Feld zaehlt hier so wenig wie ein Angebot —

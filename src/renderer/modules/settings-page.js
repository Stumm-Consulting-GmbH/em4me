...
async function readAttachmentsFromConfig() {
  let config;
  try {
...
function renderAttachmentsSection(container, draft) {
...
function renderAttachmentsAreaSection(container, draft) {
  const values = draft.attachments;
  if (!values) return;
...
function validateAttachmentsSection(draft) {
  const values = draft.attachments;
  if (!values) return null;
  return validateAttachmentsPart(values.global, ATTACHMENT_FORMS_WITH_NAME);
}

function validateAttachmentsAreaSection(draft) {
  const values = draft.attachments;
...
async function applyAttachmentsSection(draft) {
  const values = draft.attachments;
  if (!values) return;
...
function dirtyAttachmentsSection(draft) {
  const values = draft.attachments;
  if (!values) return false;
  const snap = draft.attachmentsSnapshot || {};
  const globalOut = { form: values.global.form, ordnername: values.global.ordnername.trim() };

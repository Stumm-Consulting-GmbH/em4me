...
// 4T-000987 (Epic 3E-000196): Task-Feld-Katalog des TASKS-Scopes, herausgelöst
...
// 4T-000502 (Epic 3E-000096): Prioritaets-Rang des Marker-Kerns fuer das
...
// 4T-000502 (Epic 3E-000096): Termin-Feld des Task-Modells -> Abfrage-Datum.
...
function resolveTaskField(lower, task) {
  ...
  if (dotIdx > 0) {
    ...
    // 4T-000508: ID-Zusatz-Felder ("hat ID" und Duplikat-Filter der
  ...
  switch (lower) {
    ...
    case 'urgency':
      // 4T-000505: Dringlichkeits-Score — vorberechnet vom Aufrufer
      // (computeUrgency mit injiziertem Bezugstag, siehe backlinks.js).
      return typeof task.urgency === 'number' ? task.urgency : null;
    case 'blocked':
      // 4T-000508: Blockierungs-Flags (offene Vorgaenger / blockiert andere),

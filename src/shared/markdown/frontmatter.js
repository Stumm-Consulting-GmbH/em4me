...
// 4T-0049 (Epic 3E-0010): js-yaml fuer YAML-Frontmatter-Parsing. Geparst
// wird mit JSON_SCHEMA (P-06/4T-0183: Kommentar an den Code angeglichen):
...
// 4T-0282 (Epic 3E-0050): yamlText (Blockinhalt ohne die Marker-Zeilen)
...
function writeFrontmatter(originalText, newData, options = {}) {
  try {
    ...
    // 4T-0069 (Epic 3E-0012): Zeilenenden-Konvention der Quelle uebernehmen.
    ...
    // 4T-0491 (Epic 3E-0093): Komplett-Uebernahme — die hier genannten NEUEN
    // Schluessel werden auch mit leerem Wert geschrieben (als bare YAML-
    // Schluessel ueber null + nullStr unten). Der 4T-0069-Churn-Schutz fuer
    ...
    if (buildFresh) {
      ...
      // 4T-0491 (Epic 3E-0093): leere Stub-Felder als bare Schluessel seeden
      ...
      //
      // 4T-0069 (Epic 3E-0012): Leer-Aequivalenz. Der Properties-Editor liest
      ...
      for (const key of newKeys) {
        ...
        if (isEmptyValue(currentValue) && isEmptyValue(newValue)) {
          // 4T-0491 (Epic 3E-0093): ein NEUES Feld aus der Komplett-Uebernahme
          ...
          // (4T-0069-Churn-Schutz), neue Nicht-Stub-Leerwerte wie bisher.
    ...
    // 4T-0069: doc.toString() schreibt nur LF; auf die EOL-Konvention der
    // Quelle normalisieren, damit Round-Trip bei CRLF-Dateien treu bleibt.
    // 4T-0491 (Epic 3E-0093): bei Komplett-Uebernahme leere Stub-Felder als
    ...
    // 4T-0069: Anzahl der Leerzeilen zwischen Frontmatter-Ende und Body

 ...
 * @param {object} deps.schliessRueckfall Stille-Wache des Schliess-Wegs (4T-001213).
...
function registerWindowsIpc(handle, deps) {
  ...
  handle('window:confirmClose', (event) => {
    ...
    if (w && !w.isDestroyed()) {
      // 4T-001213 (Epic 3E-000225): Die Quittung ist da, die Stille-Wache des
  ...
  // M-01 (4T-000173): Renderer signalisiert, dass der Nutzer das Schliessen
  ...
  handle('window:cancelClose', (event) => {
    setQuitting(false);
    // 4T-001213 (Epic 3E-000225): Der Anwender hat abgebrochen; die Wache endet
    ...
    // 4T-000322: laufende Bereich-Schliessen-Kaskade abbrechen.
    cancelCascade();
  });

  // --- Arbeitsbereiche: Lebenszyklus (4T-000537, Epic 3E-000098) -----------------
  // Benannte, mehrfach abgelegte logische Applikationen (Workshop-Protokoll
  // in 4T-000536). Nach jeder Ablage-Aenderung geht 'workspaces:changed' an
  // alle Fenster (Muster journals:changed); die UI (4T-000538) zieht Untermenue
  ...
  handle('workspace:saveAs', (event, params) => {
    ...
    broadcastDisplayInfo();
    workspacesChanged();
    // 4T-000630 (Epic 3E-000102): Bestands-Fenster der benannten App faerben.
    updateAllCaptionColors();
  ...
  // Oeffnen bzw. Fokussieren — Kern in openWorkspaceById (4T-000538: auch
  // Menue-Action der Untermenue-Liste).
  handle('workspace:open', (event, id) => openWorkspaceById(id, senderWindow(event)));

  // 4T-000538: Loesch-Bestaetigung als nativer Dialog (Muster
  ...
  handle('workspace:setColor', (event, params) => {
    ...
    persistAllWindows();
    workspacesChanged();
    // 4T-000630 (Epic 3E-000102): offener Arbeitsbereich — alle seine Fenster
  ...
  handle('workspace:delete', (event, id) => {
    ...
    // 4T-000539 (Epic 3E-000098): liegende Entwuerfe des geloeschten
    ...
    persistAllWindows();
    workspacesChanged();
    // 4T-000630 (Epic 3E-000102): Degradierung zur unbenannten App -> Fenster
    // zurueck auf die Standard-Titelleiste.
    updateAllCaptionColors();
    return { ok: true };
  });

  // 4T-000368 (Epic 3E-000068): Renderer meldet beim Schliessen die Unbenannt-Tabs
  ...
  // 4T-000927: Entwickler-Werkzeuge umschalten — seit dem Entfall des
  ...
  // M-16 (4T-000173): zusaetzlich debounced in den Store flushen (bestehender
  ...
  // (4T-000012). Wird vom Renderer bei jedem updateWindowTitle gesendet.
  ...
  // synchron abgefragt (4T-000012). 4T-000318: displayNumber ist app-lokal;
  ...
  handle('window:list', () => {
    ...
    for (const [id] of windows) {
      ...
      list.push({
        ...
        // 4T-000538 (Epic 3E-000098): Arbeitsbereichs-Name fuer eindeutige
        // Ziel-Labels im Tab-Kontextmenue.
        workspaceName: info.workspaceName || null,
        // 4T-000871/4T-000873: Buch- bzw. Regal-Name fuer die Ziel-Labels.
        bookName:
  ...
  // Tab in der aktiven Pane hinzu (4T-000012). payload = { path, content, dirty,
  ...
  handle('tab:appendToWindow', (_event, params) => {
    ...
    // 4T-000323 (Epic 3E-000058): kein Tab mit Datei ausserhalb des Bereichs in
  ...
  // R4-03 (4T-000170): optionaler initialTabPayload (Format wie
  ...
  handle('window:openNew', (event, initialPanes, initialTabPayload) => {
    ...
    // 4T-000318: "Neues Fenster" bleibt in der Applikation des Absenders.
    const senderAppId =

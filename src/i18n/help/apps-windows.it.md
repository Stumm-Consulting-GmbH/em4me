...
«Visualizza → Statistiche dell'area» apre una pagina di indicatori dell'area aperta come scheda dedicata; lo stesso punto di accesso si trova nel menu contestuale del pannello dell'area. La pagina è di sola lettura e mostra sei sezioni: **File e spazio occupato** (file Markdown e non Markdown suddivisi in immagini, PDF e altri, numero di cartelle, spazio occupato con le sue quote), **Proprietà** e **Tag** (il numero di file per voce, ordinabile per nome o per numero), **File di accompagnamento** (il `.mdd` di ogni documento e i file dell'area `.mdda`), **Contenuto** (attività per stato, collegamenti wiki e Markdown, alias, file senza collegamenti in entrata) e **File notevoli** (i più grandi, i modificati più di recente, i più collegati e i file senza collegamenti in entrata). Un clic su un nome di file in queste quattro liste apre il file.
...
La lista «Senza collegamenti in entrata» è la controprova dell’indicatore omonimo nella sezione «Contenuto»: elenca ogni file verso cui nell’area nessuno rimanda. È l’unica delle quattro liste a essere **completa** invece che limitata a dieci voci — alla domanda su che cosa resti isolato, una risposta troncata non sarebbe una risposta. Il valore dell’indicatore è cliccabile e porta alla lista; da tastiera il tabulatore lo raggiunge e Invio attiva il salto. Se l’area non ha file di questo tipo, la lista resta vuota e il valore senza effetto.

**La pagina iniziale dell’area non viene conteggiata qui.** Se è impostata una pagina iniziale (vedi la sezione seguente), non compare né nella lista né nell’indicatore: è il punto di ingresso dell’area e non ha bisogno di un collegamento che punti a essa. **L’indicatore risulta quindi inferiore di uno rispetto alle versioni precedenti**: è la correzione di un conteggio, non un file perduto. In tutte le altre cifre la pagina iniziale resta un file come gli altri. Un collegamento da un’area collegata continua a non contare; il conteggio si ferma al confine dell’area.

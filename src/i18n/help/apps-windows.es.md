...
- **Pasar el ratón en lugar de expandir:** al pasar el ratón por la cabecera de un grupo contraído aparece, tras una breve pausa, la lista de sus pestañas; un clic en ella cambia a ese archivo sin expandir el grupo. La pestaña activa está marcada en la lista y los archivos sin guardar llevan su punto de modificación. En el borde derecho de cada entrada hay una cruz que cierra el archivo; con cambios sin guardar aparece la misma pregunta que al cerrar en la barra de pestañas. La lista permanece abierta y muestra los archivos restantes, de modo que se pueden cerrar varios seguidos; con el último desaparece el grupo.
...
La lista «Sin enlaces entrantes» es la contraprueba del indicador del mismo nombre en la sección «Contenido»: nombra cada archivo al que nada enlaza dentro del área. Es la única de las cuatro listas que está **completa** en lugar de limitada a diez entradas: a la pregunta de qué queda suelto, una respuesta recortada no sería una respuesta. El valor del indicador se puede pulsar y salta a la lista; con el teclado, el tabulador llega hasta él e Intro activa el salto. Si el área no tiene ningún archivo así, la lista queda vacía y el valor sin efecto.

**La página de inicio del área no se cuenta aquí.** Si hay una página de inicio definida (véase la sección siguiente), no aparece ni en la lista ni en el indicador: es el punto de entrada del área y no necesita un enlace que apunte hacia ella. **Por eso el indicador es una unidad menor que en versiones anteriores**: es la corrección de un recuento, no la pérdida de un archivo. En todas las demás cifras la página de inicio sigue siendo un archivo como cualquier otro. Un enlace desde un área vinculada sigue sin contar; el recuento termina en el límite del área.

...
- **Hover instead of expanding:** hovering the header of a collapsed group brings up a list of its tabs after a short delay; a click in it switches to that file without expanding the group. The active tab is marked in the list, and unsaved files carry their change dot. At the right edge of each entry sits a cross that closes the file; for unsaved changes the same prompt appears as when closing in the tab bar. The list then stays open and shows the remaining files, so several can be closed one after another; with the last one the group disappears.
...
The list **"Without incoming links"** is the counter-check to the figure of the same name in the "Content" section: it names every file that nothing in the area links to. It is the only one of the four lists that is **complete** rather than capped at ten entries — a shortened answer to the question of what is left dangling would be no answer at all. The value of the figure is clickable and jumps to the list; with the keyboard, Tab reaches it and Enter triggers the jump. If the area has no such file, the list stays empty and the value does nothing.

**The start page of the area is not counted here.** If a start page is set (see the next section), it appears neither in the list nor in the figure: it is the entry point of the area and needs no link pointing to it. **The figure is therefore one lower than in earlier versions** — that is a correction of the count, not a lost file. In every other figure the start page remains a file like any other. A link from a linked area still does not count; the count ends at the area boundary.

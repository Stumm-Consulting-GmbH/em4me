...
- **Survoler au lieu de déplier :** survoler l'en-tête d'un groupe replié fait apparaître, après un court délai, la liste de ses onglets ; un clic dedans passe à ce fichier sans déplier le groupe. L'onglet actif est marqué dans la liste et les fichiers non enregistrés portent leur point de modification. Au bord droit de chaque entrée se trouve une croix qui ferme le fichier ; en cas de modifications non enregistrées, la même question apparaît que lors d'une fermeture dans la barre d'onglets. La liste reste ensuite ouverte et montre les fichiers restants, de sorte que plusieurs peuvent être fermés à la suite ; avec le dernier, le groupe disparaît.
...
La liste « Sans lien entrant » est la contre-épreuve de l’indicateur du même nom dans la section « Contenu » : elle nomme chaque fichier vers lequel rien ne pointe dans la zone. C’est la seule des quatre listes à être **complète** au lieu d’être limitée à dix entrées — à la question de savoir ce qui reste isolé, une réponse tronquée n’en serait pas une. La valeur de l’indicateur est cliquable et mène à la liste ; au clavier, Tabulation y accède et Entrée déclenche le saut. Si la zone ne contient aucun fichier de ce type, la liste reste vide et la valeur sans effet.

**La page de démarrage de la zone n’est pas comptée ici.** Si une page de démarrage est définie (voir la section suivante), elle n’apparaît ni dans la liste ni dans l’indicateur : elle est le point d’entrée de la zone et n’a pas besoin d’un lien qui pointe vers elle. **L’indicateur est donc inférieur d’une unité par rapport aux versions précédentes** — il s’agit d’une correction du décompte, non d’un fichier perdu. Dans tous les autres chiffres, la page de démarrage reste un fichier comme les autres. Un lien provenant d’une zone liée ne compte toujours pas ; le décompte s’arrête à la frontière de la zone.

...
Every action of the app is a command in the central registry. Command placement turns commands into permanent custom access points: command buttons in the status bar, a hide list for the default buttons, custom entries in the editor context menu and macros as command sequences. This is maintained in two places: the two status bar lists under "File → Settings… → Status bar", the context menu entries and the macros under "File → Settings… → Context menu and macros". The four functions belong to the switchable "Command placement" extension (category Tools).

## Status bar buttons

Custom command buttons appear as their own segment in the status bar, to the right of the view buttons. Creation takes three steps: choose a command via filter search, pick an icon from the curated internal set, optionally assign a display name. The button's tooltip shows the display name followed by the original command in parentheses; without a display name it shows the command itself. In the list of the "Status bar" section, buttons can be reordered (up/down), edited and removed.

If the status bar runs out of space — for example with narrow windows — the buttons move into the edge menus of the bar together with its other elements and can still be run from there; see [Views and display](views-display.md).
...
Hiding only clears away the access point, the function itself remains: everything hidden stays reachable through the menu, the command palette and keyboard shortcuts. The "Show all" button restores the default status bar. Hiding here is permanent and independent of whether a button would be usable at the moment; how the bar handles buttons that can do nothing at the moment is described by [Views and display](views-display.md).

## Editor context menu

Custom command entries appear as an additional section at the end of the editor context menu, in source as well as live mode. They are maintained in the list of the "Context menu and macros" section — same creation flow and same entry model as the status bar buttons, but with their own order. Each entry shows its icon and its display name.
...
When the "Command placement" extension is disabled, the status bar shows the default state again: no custom buttons, no hidden buttons, no context menu section; the macro commands are deregistered. In the settings, the "Context menu and macros" section is gone, and in the "Status bar" section the two lists disappear; the setting for collapsing the bar stays there, because it does not belong to the extension. The entire configuration stays saved and applies unchanged after re-enabling.

...
La función pertenece a las [extensiones internas](extensions.md) («Vista de lienzo»). Desactivada, el bloque sigue siendo un bloque de código corriente, el modo de vista desaparece y los comandos de superficie y tarjeta se van. El documento permanece legible sin cambios; no se pierde nada.
...
- Un **clic** selecciona una tarjeta, un clic en el fondo anula la selección. Como máximo hay un elemento seleccionado: una tarjeta o una conexión.
...
La superficie sigue la editabilidad de su documento. Mientras el documento esté en simple visualización, sin el modo de edición activado, la superficie es **solo de consulta**: sin tiradores, sin arrastre, sin creación, sin entrada de texto, sin barra de herramientas, y el menú contextual queda sin entradas. Desplazar la porción, ampliar y seleccionar un elemento con un clic siguen permitidos, porque no tocan el documento.
...
| Renderizada | **el bloque resumen**: tipo, volumen en tarjetas y conexiones, una vista previa de los textos de las tarjetas y el botón «Abrir la vista Canvas» |
...
El **orden dentro del bloque es a la vez el orden de apilamiento**: lo que está más abajo queda más adelante.
...
- Una tarjeta lleva **texto propio**; no hay otros tipos de tarjeta. Las formas geométricas y los marcos de grupo no forman parte de la superficie.
